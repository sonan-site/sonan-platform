...
comment on table public.attachments is 'الملفات المرفوعة: المسار والمالك والكيان المرتبط.';

create index idx_attachments_owner on public.attachments (owner_id) where deleted_at is null;
create index idx_attachments_entity on public.attachments (entity_table, entity_id) where deleted_at is null;
...
create policy attachments_read on public.attachments
  ...
  using (owner_id = (select auth.uid()) or public.fn_has_permission('attachments.read'));
...
create policy attachments_update on public.attachments
  ...
  using (owner_id = (select auth.uid()) or public.fn_has_permission('attachments.write'))
  with check (owner_id = (select auth.uid()) or public.fn_has_permission('attachments.write'));
...
create trigger trg_notification_kinds_updated_at before update on public.notification_kinds
  for each row execute function public.fn_set_updated_at();
...
create table public.notifications (
  ...
  kind         text not null references public.notification_kinds (code) on delete restrict,

...
| الطبقة | الملف | حكمها |
|---|---|---|
| المعايير التقنية | `C:\dev\standards\platform.md` · `ui-ux.md` · `completeness-contract.md` · `seed-blueprint.md` | **مُلزِمة**. لا تُنسخ هنا. مخالفة أي بند = عيب بناء يرفضه الفحص، إلا بقرار مسجَّل في `docs/adr/` بمبرره وبديله المرفوض |
| قواعد الواجهة الخاصة | `docs/UI-UX.md` | ما يخصّ سنن وحدها من الواجهة، ويشير إلى القاعدة العامة في `ui-ux.md` |
...
| قاعدة واجهة | عامة لكل منصة: `C:\dev\standards\ui-ux.md` (ق-N) · خاصة بسنن: `docs/UI-UX.md` (س-N) |
...
## ٤٫٥. الهجرة تحمل ثلاثة لا اثنين

**البنية · السياسات · المنح** — بهذا الترتيب، فالمنحة تتبع السياسة لا تسبقها.

`adr/0023`: الجدول يُمنح لدور تطبيقي **الأمر الذي له سياسة لذلك الدور ولا شيء سواه**. ولا يُعوَّل على منح افتراضية — الاعتماد عليها عطَّل الشاشات الإدارية كلها خمس عشرة هجرة بلا أن يكشفه اختبار.

**النمط:** المنح تبدأ بـ`revoke all on <الجداول الجديدة> from anon, authenticated, service_role`، ثم تمنح ما تحته سياسة. السحب أولاً لأن منح Supabase الافتراضية تسبقك إلى الجدول — `TRUNCATE` منها، وهي **لا تخضع لأمن الصفوف**.

`lib/db/grants.db-test.ts` يُفشل البناء عند أول جدول يُنسى — وقد أمسك أول جدول بعد سنّ القاعدة.

## ٤٫٦. ملاحظة الواجهة تُصلَح وتُسجَّل

**كل ملاحظة واجهة أو تجربة من الراعي تُصلَح، ثم تُسجَّل قاعدةً قبل الرفع** — في نفس الدفعة لا بعدها:

1. صلحت لكل منصة؟ ← `C:\dev\standards\ui-ux.md` بقالبه (القاعدة · لماذا · منذ · صحيح/خطأ · القسم).
2. تخصّ سنن وحدها؟ ← `docs/UI-UX.md`، مع الإشارة إلى القاعدة العامة التي تطبّقها أو تخالفها.
3. تُبلَّغ للراعي في الردّ: رقم القاعدة وأين سُجّلت.

**ولماذا:** الإصلاح وحده يعالج شاشة واحدة، فتعود الملاحظة نفسها في الشاشة التالية. والقاعدة تُقرأ قبل بناء كل شاشة جديدة.
